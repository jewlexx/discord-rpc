...
def get_signtool():
    ...
    if PLATFORM == "win":
        sdk_dir = (
            "c:\\Program Files (x86)\\Windows Kits\\10"  # os.environ['WindowsSdkDir']
        )
...
def unity(ctx):
    ...
    UNITY_PROJECT_PATH = os.path.join(
        SCRIPT_PATH, "examples", "button-clicker", "Assets", "Plugins"
    )

    if sys.platform.startswith("win"):
        LIBRARY_NAME = "discord-rpc.dll"
        BUILD_64_BASE_PATH = os.path.join(
            SCRIPT_PATH, "builds", "win64-dynamic", "src", "Release"
        )
        UNITY_64_DLL_PATH = os.path.join(UNITY_PROJECT_PATH, "x86_64")
        BUILDS.append({BUILD_64_BASE_PATH: UNITY_64_DLL_PATH})

        BUILD_32_BASE_PATH = os.path.join(
            SCRIPT_PATH, "builds", "win32-dynamic", "src", "Release"
        )
        ...
        os.rename(
            os.path.join(BUILD_BASE_PATH, "libdiscord-rpc.dylib"),
            os.path.join(BUILD_BASE_PATH, "discord-rpc.bundle"),
        )
        ...
        os.rename(
            os.path.join(BUILD_BASE_PATH, "libdiscord-rpc.so"),
            os.path.join(BUILD_BASE_PATH, "discord-rpc.so"),
        )
...
def unreal(ctx):
    ...
    UNREAL_PROJECT_PATH = os.path.join(
        SCRIPT_PATH, "examples", "unrealstatus", "Plugins", "discordrpc"
    )
    UNREAL_INCLUDE_PATH = os.path.join(
        UNREAL_PROJECT_PATH, "Source", "ThirdParty", "DiscordRpcLibrary", "Include"
    )
    mkdir_p(UNREAL_INCLUDE_PATH)
    shutil.copy(
        os.path.join(SCRIPT_PATH, "include", "discord_rpc.h"), UNREAL_INCLUDE_PATH
    )
    ...
    if sys.platform.startswith("win"):
        ...
        UNREAL_64_DLL_PATH = os.path.join(
            UNREAL_PROJECT_PATH, "Source", "ThirdParty", "DiscordRpcLibrary", "Win64"
        )
        ...
        UNREAL_32_DLL_PATH = os.path.join(
            UNREAL_PROJECT_PATH, "Source", "ThirdParty", "DiscordRpcLibrary", "Win32"
        )
        ...
        BUILD_BASE_PATH = os.path.join(SCRIPT_PATH, "builds", "osx-dynamic", "src")
        UNREAL_DLL_PATH = os.path.join(
            UNREAL_PROJECT_PATH, "Source", "ThirdParty", "DiscordRpcLibrary", "Mac"
        )
        ...
        BUILD_BASE_PATH = os.path.join(SCRIPT_PATH, "builds", "linux-dynamic", "src")
        UNREAL_DLL_PATH = os.path.join(
            UNREAL_PROJECT_PATH, "Source", "ThirdParty", "DiscordRpcLibrary", "Linux"
        )
...
def archive():
    ...
    archive_file_path = os.path.join(
        SCRIPT_PATH, "builds", "discord-rpc-%s.zip" % get_platform()
    )
